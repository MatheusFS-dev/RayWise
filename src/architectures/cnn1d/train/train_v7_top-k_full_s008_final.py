...
#                                Run Parameters                                #
# ———————————————————————————————————————————————————————————————————————————— #
# Fixed epoch count from the matching NAS-v7 top-1 trial convergence point.
EPOCHS = 33
BATCH_SIZE = 64
...
# Final protocol mode:
# - Always train on full S008 with no validation split.
# - Keep S009 strictly for one-shot locked testing after training.
...
try:
    ...
    # Enforce the final protocol: fixed-epoch training on full S008 with no
    # validation-driven callbacks, keeping S009 as a locked test set.
    if USE_VALIDATION_SPLIT:
        raise ValueError("Final protocol requires USE_VALIDATION_SPLIT=False.")

    manual_callbacks: list[callbacks.Callback] = [callbacks.TerminateOnNaN()]
