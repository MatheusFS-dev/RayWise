    ...
    **kwargs: Any,
    ...
    try:
        ...
        if "minimize" in DIRECTION:
            # The best epoch is the one with the lowest validation top-k accuracy.
            best_idx = int(np.argmin(history.history[f"val_top_{TOP_K_METRIC}_accuracy"]))
        else:
            # The best epoch is the one with the highest validation top-k accuracy.
            best_idx = int(np.argmax(history.history[f"val_top_{TOP_K_METRIC}_accuracy"]))
