...
def make_class_balanced_focal_loss(class_counts, *, beta=0.999, gamma=2.0, num_classes=2):
    """
    Build class-balanced focal loss using the "effective number of samples".

    How it works:
    1) Compute per-class weights with the effective-number formula:
       w_c = (1 - beta) / (1 - beta**n_c), where n_c is the sample count of class c.
       This increases weight for underrepresented classes and decreases it for frequent ones.
    2) Normalize weights so their sum equals num_classes, keeping the loss scale stable.
    3) For each sample, compute p_t = predicted prob of the true class.
    4) Apply focal modulation: (1 - p_t)**gamma, which downweights easy examples.
    5) Multiply by the class weight of the true class (alpha term), then average.

    Inputs:
    - class_counts: array-like of class counts (length = num_classes).
    - beta: controls how fast weights grow for rarer classes (close to 1.0 = stronger reweighting).
    - gamma: focal focusing parameter (larger = more focus on hard examples).
    - num_classes: number of classes (used for one-hot and weight normalization).
    """
    class_counts = tf.cast(class_counts, tf.float32)
    effective_num = 1.0 - tf.pow(beta, class_counts)
    weights = (1.0 - beta) / tf.maximum(effective_num, 1e-8)
    weights = weights / tf.reduce_sum(weights) * float(num_classes)

    def loss(y_true, y_pred):
        """Compute class-balanced focal loss with per-sample alpha."""
        y_true = tf.cast(y_true, tf.int32)
        y_true_oh = tf.one_hot(y_true, depth=num_classes)
        y_pred = tf.clip_by_value(tf.cast(y_pred, tf.float32), 1e-7, 1.0 - 1e-7)
        p_t = tf.reduce_sum(y_true_oh * y_pred, axis=-1)
        alpha_t = tf.reduce_sum(y_true_oh * weights, axis=-1)
...
def make_class_balanced_binary_focal_loss(class_counts, *, beta=0.999, gamma=2.0):
    """
    Build class-balanced focal loss for a single-logit sigmoid head.

    This mirrors make_class_balanced_focal_loss but assumes y_pred is a
    scalar sigmoid probability for class 1 (NLOS) and y_true is 0/1.
    The per-class weights are computed from class_counts and applied as
    alpha in the focal term.
    """
    class_counts = tf.cast(class_counts, tf.float32)
    effective_num = 1.0 - tf.pow(beta, class_counts)
    weights = (1.0 - beta) / tf.maximum(effective_num, 1e-8)
    weights = weights / tf.reduce_sum(weights) * 2.0
    alpha_neg = weights[0]
    alpha_pos = weights[1]
    ...
    def loss(y_true, y_pred):
        ...
        alpha_t = tf.where(tf.equal(y_true, 1.0), alpha_pos, alpha_neg)
        focal = -alpha_t * tf.pow(1.0 - p_t, gamma) * tf.math.log(p_t)
        return tf.reduce_mean(focal)
...
def apply_conv_normalization(x, norm_type, name_prefix):
    """Apply normalization picked by Optuna to a conv block."""
    if norm_type == "batch":
        return layers.BatchNormalization(name=f"{name_prefix}_bn")(x)
    if norm_type == "group":
        # Calculate the x input shape and print division for groups
        # print("x shape for group norm:", x.shape)
        # print("Dividing channels:", x.shape[-1], "/ 8 =", x.shape[-1] // 8)

        return layers.GroupNormalization(groups=8, axis=-1, name=f"{name_prefix}_gn")(x)
    return layers.LayerNormalization(axis=-1, name=f"{name_prefix}_ln")(x)


def apply_activation(x, activation_name, name_prefix):
    """Apply activation when requested; otherwise keep linear."""
    if activation_name == "none":
        return x
    activation_fn = tf.keras.activations.get(activation_name)
    return layers.Activation(activation_fn, name=f"{name_prefix}_act")(x)
...
class BinaryBalancedAccuracy(tf.keras.metrics.Metric):
    """
    Balanced accuracy for binary labels with a sigmoid output.
    This mirrors SparseBalancedAccuracy but with a single sigmoid output.
    """
...
def build_model(
    ...
    **kwargs: Any,
    ...
    """Build the model with narrowed search space, focal loss, and flexible norm."""
    ...
    for i in range(num_conv_layers):
        if i == 0:
            filters_choices = [128, 256]
            kernel_h_range = (3, 4)
            kernel_w_range = (3, 4)
            act_choices = ["silu", "tanh", "sigmoid", "relu", "none"]
            pool_size = (3, 4)
        elif i == 1:
            filters_choices = [128, 256]
            kernel_h_range = (3, 4)
            kernel_w_range = (5, 6)
            act_choices = ["elu", "silu", "relu", "none"]
            pool_size = (2, 4)
        else:
            filters_choices = [64, 96, 128]
            kernel_h_range = (5, 6)
            kernel_w_range = (3, 4)
            act_choices = ["none", "silu", "sigmoid"]
            pool_size = (2, 4)

        filters = trial.suggest_categorical(f"conv2d_{i}_filters", filters_choices)
        kernel_h = trial.suggest_int(f"conv2d_{i}_kernel_h", kernel_h_range[0], kernel_h_range[1])
        kernel_w = trial.suggest_int(f"conv2d_{i}_kernel_w", kernel_w_range[0], kernel_w_range[1])
        act_name = trial.suggest_categorical(f"conv2d_{i}_act", act_choices)
        ...
        x = apply_conv_normalization(x, norm_type, f"conv2d_{i}")
        x = apply_activation(x, act_name, f"conv2d_{i}")
    ...
    for i in range(coord_layers):
        coord_units = trial.suggest_int(f"coord_dense_{i}_units", 64, 96, step=16)
        coord_dropout = trial.suggest_float(f"coord_dense_{i}_dropout", 0.0, 0.3, step=0.1)
        coord_act = trial.suggest_categorical(
            f"coord_dense_{i}_act",
            ["relu", "none"],
        )
        coord = layers.Dense(
            coord_units,
            kernel_initializer=initializer,
            name=f"coord_dense_{i}",
        )(coord)
        coord = apply_activation(coord, coord_act, f"coord_dense_{i}")
        coord = layers.Dropout(coord_dropout, name=f"coord_dense_{i}_dropout")(coord)
    ...
    for i in range(fusion_layers):
        fusion_units = trial.suggest_int(f"fusion_dense_{i}_units", 64, 128, step=32)
        fusion_dropout = trial.suggest_float(f"fusion_dense_{i}_dropout", 0.1, 0.4, step=0.1)
        fusion_act = trial.suggest_categorical(
            f"fusion_dense_{i}_act",
            ["relu", "elu", "none"],
        )
        x = layers.Dense(
            fusion_units,
        ...
        x = apply_activation(x, fusion_act, f"fusion_dense_{i}")
        x = layers.Dropout(fusion_dropout, name=f"fusion_dense_{i}_dropout")(x)
    ...
    if head_type == "sigmoid":
        outputs = layers.Dense(
            ...
            name="output",
            kernel_initializer=initializer,
            bias_initializer=initializers.Constant(output_bias) if output_bias is not None else "zeros",
        )(x)
        balanced_metric = BinaryBalancedAccuracy(name="balanced_accuracy")
    else:
        outputs = layers.Dense(
            2,
            activation="softmax",
        ...
        balanced_metric = SparseBalancedAccuracy(num_classes=2, name="balanced_accuracy")
    ...
    learning_rate = trial.suggest_float("learning_rate", 4e-4, 8e-4, log=True)
    optimizer = optimizers.AdamW(learning_rate=learning_rate, weight_decay=1e-4)

    model.compile(
        optimizer=optimizer,
        loss=loss_fn if loss_fn is not None else losses.SparseCategoricalCrossentropy(),
        metrics=[
            "accuracy",
            balanced_metric,
...
def objective(
    ...
    **kwargs: Any,
    ...
    try:
        ...
        # Class-balanced focal loss replaces class_weighted CE.
        head_type = trial.suggest_categorical("output_head", ["softmax", "sigmoid"])

        if head_type == "sigmoid":
            focal_loss = make_class_balanced_binary_focal_loss(class_counts)
            prior_los = float(class_counts[LABEL_LOS]) / float(total_samples)
            target_prior_los = min(max(prior_los, 0.6), 0.99)
            target_prior_nlos = 1.0 - target_prior_los
            output_bias = float(np.log(target_prior_nlos / target_prior_los))
        else:
            focal_loss = make_class_balanced_focal_loss(class_counts)
            # Bias output logits toward LOS to counter NLOS-heavy predictions.
            prior_los = float(class_counts[LABEL_LOS]) / float(total_samples)
            target_prior_los = min(max(prior_los, 0.6), 0.99)
            output_bias = [np.log(target_prior_los), np.log(1.0 - target_prior_los)]
        ...
        #                              Confusion Matrices                              #
        # ———————————————————————————————————————————————————————————————————————————— #
        def predict_labels(x_lidar, x_coord):
            preds = model.predict(
                x=[x_lidar, x_coord],
                batch_size=batch_size,
                verbose=0,
        ...
        y_pred_train = predict_labels(x_lidar_train, x_coord_train)
        ...
        y_pred_val = predict_labels(x_lidar_val, x_coord_val)
        plot_confusion_matrix(
            y_true=y_val,
            y_pred=y_pred_val,
        ...
        y_pred_s009 = predict_labels(s009_lidar_input, s009_coord_input_scaled)
        plot_confusion_matrix(
