...
def build_model(trial: optuna.Trial, hparams: dict, show_summary: bool = True, t_seed: int = 42) -> tf.keras.Model:
    ...
    # ———————————————————————————————— Conv Layers ——————————————————————————————— #
    conv_layers = trial.suggest_int("conv_layers", 4, 8)
    max_pool_dim1 = math.floor((20 * 200) ** (1.0 / conv_layers))

    for i in range(conv_layers):
        filters = trial.suggest_categorical(f"conv1d_{i}_filters", [32, 64, 128, 256, 512])
    ...
    dense_layer_type = trial.suggest_categorical(
        "dense_layer_type",
        [
            "dense",
            "conv1x1",
    ...
    if dense_layer_type == "dense":
    ...
    elif dense_layer_type == "conv1x1":
        for i in range(dense_layers):
            filters = trial.suggest_categorical(f"dense_conv1x1_{i}_filters", [32, 64, 128, 256, 512])
            x = build_dense_as_conv1d(
                name_prefix=f"dense_conv1x1_{i}",
                trial=trial,
                hparams=hparams,
                x=x,
                # Filters
        ...
        x = layers.Flatten(name="flatten_dense_conv1x1")(x)
