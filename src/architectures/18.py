...
def build_model(trial: optuna.Trial, hparams: dict, show_summary: bool = True, t_seed: int = 42) -> tf.keras.Model:
    ...
    # ————————————————————————————————— Attention ———————————————————————————————— #
    x = layers.LayerNormalization(name="ln_pre_attn")(x)  # → (batch,L2,f2)
    attn = layers.MultiHeadAttention(
        num_heads=trial.suggest_int("attn_heads", 2, 4),
        key_dim=trial.suggest_categorical("attn_key_dim", [32, 64]),
        dropout=trial.suggest_float("attn_dropout", 0.0, 0.3, step=0.1),
        name="mhsa",
    )(
        x, x
    )  # → (batch,L2,f2)
    x = layers.Add(name="residual_attn")([x, attn])  # → (batch,L2,f2)
    x = layers.LayerNormalization(name="ln_post_attn")(x)  # → (batch,L2,f2)
