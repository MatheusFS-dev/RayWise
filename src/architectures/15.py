...
def build_model(trial: optuna.Trial, hparams: dict, show_summary: bool = True, t_seed: int = 42) -> tf.keras.Model:
    ...
    # project up to d_model, since it's too small (C=6)
    d_model = trial.suggest_categorical("d_model", [32, 64, 128, 256])
    projection = layers.Dense(d_model, name="projection")(combined)
    ...
    attn = layers.MultiHeadAttention(
        ...
        value=projection,
        key=projection,
    )  # (query, value, key=Default [Value])

    x = layers.Add(name="residual_attention")([projection, attn])
